...
void prepareTxData(TestPara_TypeDef* pTestPara, TestResult_TypeDef* pTestResult, uint8_t* pUartTxBuf)
{
	...
	*(pUartTxBuf+2)=(uint8_t)((pTestPara->V_Now)>>24);
	*(pUartTxBuf+3)=(uint8_t)((pTestPara->V_Now)>>16);
	*(pUartTxBuf+4)=(uint8_t)((pTestPara->V_Now)>>8);
	*(pUartTxBuf+5)=(uint8_t)((pTestPara->V_Now)&0xFF);
	*(pUartTxBuf+6)=(uint8_t)((pTestPara->I_Now)>>24);
	*(pUartTxBuf+7)=(uint8_t)((pTestPara->I_Now)>>16);
	*(pUartTxBuf+8)=(uint8_t)((pTestPara->I_Now)>>8);
	*(pUartTxBuf+9)=(uint8_t)((pTestPara->I_Now)&0xFF);
	*(pUartTxBuf+10)=pTestResult->V_avg.numUchar[3];																		
	*(pUartTxBuf+11)=pTestResult->V_avg.numUchar[2];																		
	*(pUartTxBuf+12)=pTestResult->V_avg.numUchar[1];																	
	*(pUartTxBuf+13)=pTestResult->V_avg.numUchar[0];
	*(pUartTxBuf+14)=pTestResult->I_avg.numUchar[3];																		
	*(pUartTxBuf+15)=pTestResult->I_avg.numUchar[2];																		
	*(pUartTxBuf+16)=pTestResult->I_avg.numUchar[1];																	
	*(pUartTxBuf+17)=pTestResult->I_avg.numUchar[0];																	
	*(pUartTxBuf+18)=0;
