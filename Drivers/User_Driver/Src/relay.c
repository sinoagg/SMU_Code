...
void SetRelay(uint8_t rangeSelect)                                   //FVMI设置采集电压放大范围，FIMV设置电流档位
{
	RelayClear();
	switch(rangeSelect)
	{
...
uint8_t RelayCheck(enum TestMode testMode, TestResult_TypeDef* pTestResult, Relay_TypeDef* pRelay)
{
	...
	else
	{
		...
		{	
			...
			{
				if(pRelay->rangeNow>pRelay->minRange)														//如果档位在1档以上，仍然可以降档
				{		
					SetRelay(--(pRelay->rangeNow));
			...
			{
				if(pRelay->rangeNow<pRelay->maxRange)	                      //如果档位在最大档以下，仍然可以升档
				{
					SetRelay(++(pRelay->rangeNow));
					HAL_Delay(RANGE_CHANGE_DELAY);
					return 1;
