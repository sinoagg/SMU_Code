...
typedef struct
{
	...
	int16_t V_Step;										//step for voltage sweeping use
	int16_t I_Step;										//step for current sweeping use
	uint8_t I_Unit;										//uint of current mA, uA, nA, pA
	uint16_t avgNumber;								//默认16个点取平均值
	uint16_t quietTime;								//默认静默时间是2s
	int16_t startDelay;								//暂时没用
	uint16_t sampleTime;							//sample total time
}TestPara_TypeDef;
